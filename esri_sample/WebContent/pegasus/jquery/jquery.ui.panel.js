	...
	$.widget(
			'ui.panel',
			{
				options : {
					...
					collapsible : true,
					expanded : false,
					expandType : 'right', // down, left, right
					expandSpeed : 'slow',
					draggable : false,
					resizable : false,
				...
				_create : function() {
					...
					if(options.customIcons != undefined){
						options.icons = options.customIcons;
					}
				...
				_createIcons : function() {
					...
					if (options.icons) {
						...
						if(options.icons.isCustomIcon){
							$('<span style="top: 10px !important"></span>')
				...
				hideMapOptionDiv: function() {
					if(this.options.expanded) {
						this.toggle(this.options.expandSpeed);
					}

		        },
		        showOptionDiv: function() {
					if(!this.options.expanded) {
						this.toggle(this.options.expandSpeed);
					}
				...
				toggle : function(expandSpeed, innerCall) {
					...
					if (options.expandType != 'down') {
						...
						if(!options.expanded) {
							self.element.animate( {width: self.titleWidth, height: '90px'}, expandSpeed);
						}else {
							self.element.animate( {width: self.originalWidth, height: '100%'}, expandSpeed);
						}
					}

...
// event handlers
var onmouseupHandler, onmousemoveHandler, onmousedownHandler, onmousedragHandler, ondblClickHandler;


//method to create kendoComboBox with specfied comboId,placeholder,filterType,dataSource,dataTextField & dataValueField
function createKendoComboBox(comboId,placeholder,filterType,dataSource,dataTextField,dataValueField){
	return createKendoComboBoxByDiv($(HASH_STRING + comboId),placeholder,filterType,dataSource,dataTextField,dataValueField);
}
...
function _createKendoDropDown(id, dataProvider, labelField, valueField, changeFunction) {
	$("#"+id).kendoComboBox({
	    minLength: 0,
...
function setStartValue(e,slider_id,startValue_id,rangeSlider,minValue,maxValue){
    if (e.type != "keypress" || kendo.keys.ENTER == e.keyCode) {
        var startValue = parseInt($("#"+startValue_id).val());

        if (isNaN(startValue) || startValue < minValue || startValue > maxValue) {
...
function createKendoRangeSlider(sliderDivId,startValue_id,endValue_id,minValue,maxValue){
	return createKendoRangeSliderByDiv($(HASH_STRING +sliderDivId),startValue_id,endValue_id,minValue,maxValue);
}

function createKendoRangeSliderByDiv(sliderDivObj,startValue_id,endValue_id,minValue,maxValue){
	var rangeSlider = sliderDivObj.kendoRangeSlider({
		tickPlacement:'none',
		min: minValue,
		max: maxValue,
		slide: function(e) {rangeSliderOnSlide(e,startValue_id,endValue_id);}
		}).data("kendoRangeSlider");
    $("#"+startValue_id).keypress(function(e) {setStartValue(e,slider_id,startValue_id,rangeSlider,minValue,maxValue);});
	$("#"+endValue_id).keypress(function(e) {setEndValue(e,slider_id,endValue_id,rangeSlider,minValue,maxValue);});
	return rangeSlider;
}
...
function validateExpandCheckbox(checkboxObj, itemId, ifEnbldMrkCheck,windowName) {
	validateExpandCheckboxByDiv(checkboxObj, $(HASH_STRING+itemId),ifEnbldMrkCheck, windowName);
}

function validateExpandCheckboxByDiv(checkboxObj, itemDivObj,ifEnbldMrkCheck, windowName) {
...
function validateComboBoxOnCheckbox(checkboxObj, comboId,comboType) {
	validateComboBoxOnCheckboxByDiv(checkboxObj, $(HASH_STRING+comboId),comboType);
}
...
function createOptionsPanel(itmeId,collapseType,isCollapsed,isTrueVerticalText,vHeight,width){
	return createOptionsPanelByDiv($(HASH_STRING +itmeId),collapseType,isCollapsed,isTrueVerticalText,vHeight,width);
}

function createOptionsPanelByDiv(panelDiv,collapseType,isCollapsed,isTrueVerticalText,vHeight,width){
...
function updateControl(controlArr,flag,dashboard){
	for(var i=0;i<controlArr.length;i++){
		if(dashboard.$("#"+controlArr[i]) && dashboard.$("#"+controlArr[i]).length > 0) {
...
function updatekendoMultiSelect(controlArr,controlType,dashboard){
	for(var i=0;i<controlArr.length;i++){
		if(dashboard.$("#"+controlArr[i]) && dashboard.$("#"+controlArr[i]).length > 0) {
			dashboard.$("#"+controlArr[i]).data(controlType).value(EMPTY_STRING);
		}
	}
}
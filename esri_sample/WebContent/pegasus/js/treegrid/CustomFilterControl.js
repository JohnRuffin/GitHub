...
(function(window) {
    "use strict";
    var CustomFilterControl, uiUtil = flexiciousNmsp.UIUtils,
    ...
    CustomFilterControl = function() {
        ...
        this.updateInnerHTML("<img src='pegasus/portalcss/treegrid/images/custom/filter.png'></img>");

        this.addEventListener(this, "click", function(b) {
            this.showPopup();
        });
    };

    myCompanyNameSpace.SpaceTime_CustomFilterControl = CustomFilterControl; //add to name space
    CustomFilterControl.prototype = new flexiciousNmsp.UIComponent("span"); //setup hierarchy
    ...
    CustomFilterControl.prototype.isMatch = function(emp) {
        ...
        if (this.filterExpression1.expression || this.filterExpression2.expression) {
            var fe1Result = true,
                fe2Result = true;
            var fe1ExpValue, fe2ExpValue;
            var value2Compare;
            if (this.filterExpression1.expression) {
                switch (this.parent.getColumn().filterComparisionType) {
                case gridFilterTypeConstants.FILTER_TYPE_TIME:
                    fe1Result = this.performTimeComparision(this.filterExpression1.columnName, this.filterExpression1.expression, emp, this.filterExpression1.filterOperation);
                    break;
                default:
            ...
            if (this.filterExpression2.expression) {
                switch (this.parent.getColumn().filterComparisionType) {
                case gridFilterTypeConstants.FILTER_TYPE_TIME:
                    fe2Result = this.performTimeComparision(this.filterExpression2.columnName, this.filterExpression2.expression, emp, this.filterExpression2.filterOperation);
                    break;
                default:
    ...
    CustomFilterControl.prototype.performTimeComparision = function(columnName, expressionValue, data, filterOperation) {
        var value2Compare = data[columnName];
        if (value2Compare != undefined) {
            value2Compare = parseInt(value2Compare.replace(":", ""));
            expressionValue = parseInt(expressionValue.replace(":", ""));
            return this.compareGridData(value2Compare, expressionValue, filterOperation);
        }
    };

    CustomFilterControl.prototype.compareGridData = function(value2Compare, fe1ExpValue, filterOperation) {
        var fe = flexiciousNmsp.FilterExpression;
        if (filterOperation == fe.FILTER_OPERATION_TYPE_EQUALS) return value2Compare == fe1ExpValue;
        if (filterOperation == fe.FILTER_OPERATION_TYPE_GREATER_THAN) return value2Compare > fe1ExpValue;
        if (filterOperation == fe.FILTER_OPERATION_TYPE_GREATERTHANEQUALS) return value2Compare >= fe1ExpValue;

        if (filterOperation == fe.FILTER_OPERATION_TYPE_LESS_THAN) return value2Compare < fe1ExpValue;
        if (filterOperation == fe.FILTER_OPERATION_TYPE_LESS_THAN_EQUALS) return value2Compare <= fe1ExpValue;
        if (filterOperation == fe.FILTER_OPERATION_TYPE_NOT_EQUALS) return value2Compare != fe1ExpValue;
    ...
    CustomFilterControl.prototype.addFilterExpressions = function() {
        ...
        var fe = flexiciousNmsp.FilterExpression;
        var numericOptions = [
        fe.FILTER_OPERATION_TYPE_EQUALS, fe.FILTER_OPERATION_TYPE_NOT_EQUALS, fe.FILTER_OPERATION_TYPE_GREATER_THAN, fe.FILTER_OPERATION_TYPE_GREATERTHANEQUALS, fe.FILTER_OPERATION_TYPE_LESS_THAN, fe.FILTER_OPERATION_TYPE_LESS_THAN_EQUALS];

        var stringOptions = [
        fe.FILTER_OPERATION_TYPE_EQUALS, fe.FILTER_OPERATION_TYPE_NOT_EQUALS, fe.FILTER_OPERATION_TYPE_BEGINS_WITH, fe.FILTER_OPERATION_TYPE_ENDS_WITH, fe.FILTER_OPERATION_TYPE_CONTAINS, fe.FILTER_OPERATION_TYPE_DOES_NOT_CONTAIN];
        var options = stringOptions;
        if (this.parent.getColumn().filterComparisionType == fe.FILTER_COMPARISION_TYPE_STRING) {
            options = stringOptions;
        } else if (this.parent.getColumn().filterComparisionType == fe.FILTER_COMPARISION_TYPE_NUMBER || this.parent.getColumn().filterComparisionType == fe.FILTER_COMPARISION_TYPE_DATE) {
            options = numericOptions;
        } else if (this.parent.getColumn().filterComparisionType == fe.FILTER_COMPARISION_TYPE_BOOLEAN) {
            options = stringOptions;
        } else if (this.parent.getColumn().filterComparisionType == 'time') {
            options = numericOptions;
    ...
    CustomFilterControl.prototype.showPopup = function(parent) {
        ...
        filterDiv.domElement.innerHTML = "Show items with value that : <br/>" + "<table><tr><td><select id='selectFilter1'>" + this.addFilterExpressions() + "</select></td><td class='multiSelectComboBoxPopupTd'>" + "<input id='valueFilter1' style='width:100px;'></td></tr>" + "<tr><td colspan='2' align='center'><select id='selectAndOr'><option label='AND'>AND</option><option label='OR'>OR</option> </select></td></tr>" + "<tr><td><select id='selectFilter2'>" + this.addFilterExpressions() + "</select></td>" + "<td class='multiSelectComboBoxPopupTd'><input id='valueFilter2' style='width:100px;'></td></tr></table>";
        ...
        pt = this.localToGlobal(pt); {
            if (!parent) parent = flexiciousNmsp.DisplayList.instance().documentComponent.domElement.body;
            else {
        ...
        if (this.filterExpression1.filterOperation) $(this.popup.domElement).find("#selectFilter1").val(this.filterExpression1.filterOperation);
        if (this.filterExpression1.expression != null) $(this.popup.domElement).find("#valueFilter1").val(this.filterExpression1.expression);

        if (this.filterExpression2.filterOperation) $(this.popup.domElement).find("#selectFilter2").val(this.filterExpression2.filterOperation);
        if (this.filterExpression2.expression != null) $(this.popup.domElement).find("#valueFilter2").val(this.filterExpression2.expression);
        $(this.popup.domElement).find("#selectAndOr").val(this.andOr);

        this.popup.addEventListener(this, flxConstants.EVENT_CLICK, function(e) {
    ...
    CustomFilterControl.prototype.getValue = function() {
        var val = "";
        if (this.filterExpression1.expression) val += (this.filterExpression1.filterOperation + "_" + this.filterExpression1.expression.toString());
        if (this.filterExpression2.expression) val += ("_" + this.andOr + "_" + this.filterExpression2.filterOperation + "_" + this.filterExpression1.expression.toString());
        return val;
    };
    /**
    ...
    CustomFilterControl.prototype.setValue = function(val) {
        ...
        if (val) {
            ...
            if (vals.length == 2) {
                ...
                filterCellRenderer = (this.parent.getGrid().getFilterContainer().getCellForRowColumn(this.parent.getGrid().getFilterContainer().rows[0].getData(), this.parent.getColumn())).getRenderer();
                if(filterCellRenderer != undefined){
                	filterCellRenderer.updateInnerHTML("<img src='" + this.getIconUrl() + "' > </img>");
                }                
    ...
    CustomFilterControl.prototype.onOkButton = function() {
        ...
        var filterCellRenderer = (this.parent.getGrid().getFilterContainer().getCellForRowColumn(this.parent.getGrid().getFilterContainer().rows[0].getData(), this.parent.getColumn())).getRenderer();
        filterCellRenderer.updateInnerHTML("<img src='" + this.getIconUrl() + "' > </img>");
    };

    CustomFilterControl.prototype.updateInnerHTML = function(html) {
        this.setInnerHTML(html);
    };

    CustomFilterControl.prototype.getIconUrl = function() {
        var filterVal = this.parent.getGrid().getFilterValue(this.parent.getColumn().dataField);
        if ((this.parent.getColumn().enableFilters == true ) ||  this.parent.getColumn().level.getEnableFilters()) {
            if (filterVal == "" || filterVal == null) {
                return this.parent.getGrid().imagesRoot + "/custom/filter.png";
            } else {
                return this.parent.getGrid().imagesRoot + "/custom/filter_applied.png";
            }
        }
        return "";
    };

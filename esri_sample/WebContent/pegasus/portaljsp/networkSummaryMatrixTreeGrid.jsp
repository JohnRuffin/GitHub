...
<head>
<title>Summary Network Matrix</title>
...
<script src="<%=contextName%>/pegasus/portaljs/networkSummaryMatrixTreeGrid.js<%=jsVersion%>" type="text/javascript" ></script>
<!-- Pegasus TreeGrid JS-->
...
<script>
	$(document).ready(function() {
		initialize();
		initalizeMouseEventsForGrouping(getContainerId()[0],$('#networkSummaryMatrixGroupBar'));
	});
...
<body style="width:100%;height:100%" oncontextmenu="return false;">
   <!-- <div id="networkSummaryMatrix" class="matrixDivStyle"> -->
   <div id="networkSummaryMatrixTreeGrid">
    	<!-- <div id="networkSummaryMatrixGroupBar" style="position: absolute;left: 10px;width: 500px;top:10px;height: 20px;background-color: #98c0f4; border: solid 1px #000000;padding: 6px;" >Drag a column here to group by that column.</div>-->
    	<div id="networkSummaryMatrixGroupBar" class="group-bar" >Drag a column here to group by that column.</div>
    	<div id="networkSummaryMatrixTreeGridDivContainer" style="position: absolute;  left: 10px;right: 10px;top:60px; bottom: 10px;"></div>
	</div>
	...
	<div id="headerButtonsBar" class="window-header k-window-actions btnBarPosition addOverflowStyle">
		<div class="btnsWrapper">
	 		...
	 		<a id="network-view" onclick="openNetworkMatrixDashboard(DASHBOARD_ID_NETWORK_MATRIX)" class="iconbtn" title="<bean:message key='header.icon.network.matrix' bundle='matrixresources'/>" href="#">
	        		<span class="k-icon open-network-matrix" >
	        		</span>
	        </a>
	        <a id="btnNetworkSummaryMatrixSyncMap" class="iconbtn n-icon" onclick="VIEWER.enableSync(this,DASHBOARD_ID_NETWORK_SUMMARY_MATRIX,[DASHBOARD_ID_MAP_VIEW,DASHBOARD_ID_SCHEMATIC_VIEW,DASHBOARD_ID_NETWORK_MATRIX,DASHBOARD_ID_NETWORK_SCHEDULE_MATRIX,DASHBOARD_ID_SCHEDULE_MATRIX], 'sync-to-map')" title="<bean:message key='header.icon.syncto.map' bundle='matrixresources'/>" href="#">
	     		<span class="k-icon sync-to-map">
	     			<bean:message key='header.icon.syncto.map' bundle='matrixresources'/>
	     		</span>
		    </a>
		    <a id="mRefresh" class="iconbtn" onclick="getDashboardContentWindow(DASHBOARD_ID_NETWORK_SUMMARY_MATRIX).refreshMatrix(true,true)" title="<bean:message key='header.icon.refresh' bundle='matrixresources'/>" href="#">
	    	...
	    	<a id="" class="iconbtn" onclick="getDashboardContentWindow(DASHBOARD_ID_NETWORK_SUMMARY_MATRIX).exportToExcel(this)" title="<bean:message key='header.icon.exportto.excel' bundle='matrixresources'/>" href="#">
	    	...
	    	<%-- <a id="toggleMZulu" onclick="getDashboardContentWindow(DASHBOARD_ID_NETWORK_SUMMARY_MATRIX).showLocalZuluColumns(this)" toggled=true class="iconbtn" title="<bean:message key='header.icon.zulu' bundle='matrixresources'/>" href="#">
	    		<span class="k-icon icon-toggle-local">
	    			<bean:message key='header.icon.zulu' bundle='matrixresources'/>
	    		</span>
	    	</a> --%>
	       	<ul id ="networkSummaryMatrixFavoritesMenu" class="iconbtn favorite-dropdown" title = "<bean:message key='header.icon.network.matrix.favorites' bundle='matrixresources'/>">
			</ul>
	       	<div class="inline-iconbtn bar-divider-container">
	   			<div class="bar-divider"></div>
	    	</div>
